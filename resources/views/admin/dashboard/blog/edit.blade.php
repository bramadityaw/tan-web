      ...
      <div class="lg:col-span-2">
        ...
        "grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-5">
          ...
          <div class="md:col-span-5">
            ...
            <select name="kategori" id="kategori" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50">
                <option value="1" {{ $article->category == '1' ? 'selected' : '' }}>Kategori 1</option>
                <option value="2" {{ $article->category == '2' ? 'selected' : '' }}>Kategori 2</option>
                <option value="3" {{ $article->category == '3' ? 'selected' : '' }}>Kategori 3</option>
            </select>
          </div>
          <div class="md:col-span-5">
            <label for="deskripsi">Deskripsi Artikel</label>
            <textarea class="rounded-l border px-3 py-2 bg-gray-50 w-full" name="deskripsi" id="deskripsi" cols="30" rows="6">{{ $article->description }}</textarea>
          </div>
          <div class="md:col-span-5">
        ...
        <div class="md:col-span-5">
           <label for="publish_at">Publish At</label>
              <input type="date" name="publish_at" id="publish_at" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" value="{{ $article->published_at }}">
        </div>
